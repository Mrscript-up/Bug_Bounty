...
def main():
    ...
    katana_merged_temp = os.path.join(temp_dir, "katana_merged_temp.txt")
    ...
    # Merge Katana outputs temporarily
    print_info("Merging Katana outputs...")
    with open(katana_merged_temp, 'w') as outfile:
    ...
    # Verify discovered Katana paths using httpx
    if os.path.exists(katana_merged_temp) and os.path.getsize(katana_merged_temp) > 0:
        print_status("Checking Katana found paths with httpx to keep only live endpoints...")
        # httpx -l katana_merged_temp.txt -silent -o all_things_in_katana.out
        subprocess.run(["httpx", "-l", katana_merged_temp, "-silent", "-o", all_katana_out], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if os.path.exists(katana_merged_temp):
            os.remove(katana_merged_temp)
    else:
        # If empty, create an empty final output file
        open(all_katana_out, 'w').close()
        if os.path.exists(katana_merged_temp):
            os.remove(katana_merged_temp)
            
    print_info("Phase 4 completed. Waiting before next phase...")
    time.sleep(2)
